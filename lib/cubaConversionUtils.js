 ...
 * @param {number} decimal - Parte decimal (ex: 0.5, 0.3, 0.1)
 * @returns {string} - Unidade da cozinha (ex: "1 cuba P", "3 potes")
 */
const convertDecimalToKitchenUnit = (decimal) => {
  ...
  // Para outros valores, tentar aproximar
  if (rounded < 0.5) {
...
export const testConversions = () => {
  ...
  console.log("=== TESTE DE CONVERSÕES CUBA-G ===");
  testCases.forEach(({ input, expected }) => {
    const result = convertCubaGToKitchenFormat(input);
    const status = result === expected ? "✅ PASS" : "❌ FAIL";
    console.log(`${input} cuba-g → "${result}" (esperado: "${expected}") ${status}`);
  });
};