...
export class RecipeCalculator {
  ...
  static calculatePreparationMetrics(preparation, allPreparations = []) {
    ...
    if (preparation.sub_components && Array.isArray(preparation.sub_components)) {
      preparation.sub_components.forEach(subComp => {
        const weight = parseNumber(subComp.weight_portioned || subComp.assembly_weight_kg);
        let cost = 0;

        if (subComp.type === 'ingredient') {
          const unitPrice = this.getUnitPrice(subComp);
          cost = weight * unitPrice;
        } else {
          // Lógica para sub-componentes que são preparações
          ...
          if (sourcePrep && sourcePrep.id !== preparation.id) {
            ...
            const sourceCostPerKg = sourceMetrics.totalYieldWeight > 0 ? sourceMetrics.totalCost / sourceMetrics.totalYieldWeight : 0;
            cost = weight * sourceCostPerKg;
          } else {
            cost = parseNumber(subComp.total_cost); // Fallback
          }
        }
  ...
  static calculateRecipeMetrics(preparations, recipeData = {}) {
    console.log("[Calculator] recalculateRecipeMetrics triggered. Preparations count:", preparations.length);
    ...
    console.log("[Calculator] Final assembly step found:", finalAssemblyStep ? finalAssemblyStep.title : 'None');
    ...
    if (finalAssemblyStep) {
      ...
      console.log("[Calculator] Calculating metrics based on final assembly step.");
      const finalStepMetrics = this.calculatePreparationMetrics(finalAssemblyStep, preparations);
      console.log("[Calculator] Metrics for final step:", JSON.stringify(finalStepMetrics, null, 2));
      ...
      console.log("[Calculator] No final assembly step found. Summing all preparations as fallback.");
    ...
    console.log(`[Calculator] Final calculated values: finalCost=${finalCost}, finalYieldWeight=${finalYieldWeight}, totalRawWeight=${totalRawWeight}`);
    ...
    console.log("[Calculator] Returning final result object:", JSON.stringify(result, null, 2));
    return result;
  }
