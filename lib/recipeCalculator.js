...
export class RecipeCalculator {
  ...
  static calculateRecipeMetrics(preparations, recipeData = {}) {
    ...
    // Ajustar métricas finais dividindo por unitsQuantity se for > 1
    if (unitsQuantity > 1) {
      finalYieldWeight = finalYieldWeight / unitsQuantity;
      finalCost = finalCost / unitsQuantity;
      totalRawWeight = totalRawWeight / unitsQuantity; // Ajustar também o peso bruto para consistência
    }

    console.log(`[Calculator] Final calculated values: finalCost=${finalCost}, finalYieldWeight=${finalYieldWeight}, totalRawWeight=${totalRawWeight}, unitsQuantity=${unitsQuantity}`);
