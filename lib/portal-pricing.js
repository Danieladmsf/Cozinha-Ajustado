...
export class PortalPricingSystem {
  ...
  static syncItemPricing(item, recipe) {
    ...
    // Log apenas para debug de preços quando necessário
    // console.log(`🔄 [PORTAL PRICING SYNC] ${recipe.name}: ${originalUnitPrice} → ${newUnitPrice}`);

    return {
      ...item,
      unit_price: newUnitPrice,
      total_price: quantity * newUnitPrice,
      // ✅ CORREÇÃO: Adicionar dados de peso das receitas
      cuba_weight: this.parseQuantity(recipe.cuba_weight) || 0,
      yield_weight: this.parseQuantity(recipe.yield_weight) || 0,
      total_weight: this.parseQuantity(recipe.total_weight) || 0
    };
  }
