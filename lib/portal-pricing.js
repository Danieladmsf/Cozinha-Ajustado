...
export class PortalPricingSystem {
  ...
  static recalculateItemUnitPrice(item, recipe, containerType = null) {
    ...
    // Se o preço calculado for zero (ex: receita sem dados de custo), 
    // mas o item original já tinha um preço, é mais seguro usar o preço antigo.
    if (finalPriceWithPolicy === 0 && this.parseQuantity(item.unit_price) > 0) {
      return this.parseQuantity(item.unit_price);
    }
