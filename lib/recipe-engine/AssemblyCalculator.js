...
export class AssemblyCalculator {
  ...
  static calculateSubComponent(subComponent, sourcePreparations = []) {
    ...
    if (assemblyWeight <= 0) {
      console.warn(`Sub-componente ${normalizedSC.name} sem peso de montagem válido`);
      return {
        ...normalizedSC,
        proportionalCost: 0,
        percentage: 0,
    ...
    if (sourceYieldWeight > 0 && sourceTotalCost > 0) {
      ...
      console.debug(`Sub-componente ${normalizedSC.name}:`, {
        assemblyWeight,
        sourceYieldWeight,
        sourceTotalCost,
        scaleFactor,
      ...
      console.warn(`Sub-componente ${normalizedSC.name}: dados insuficientes para cálculo`, {
        sourceYieldWeight,
        sourceTotalCost
      });
    }
