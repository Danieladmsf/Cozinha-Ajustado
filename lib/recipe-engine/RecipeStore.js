...
const useRecipeStore = create((set, get) => ({
        ...
        try {
          ...
          state.preparations.forEach(prep => {
            prep.ingredients.forEach(ing => {
              const cost = parseFloat(ing.current_price) || 0; // Use current_price
          ...
          return {
            metrics: {
              ...
              cost_per_kg_raw: totalWeight > 0 ? totalCost / (totalWeight / 1000) : 0
            },
  ...
  dispatch: async (action) => {
    ...
    try {
      switch (action.type) {
        ...
        default:
          console.warn(`Action type '${action.type}' não reconhecida`);
          return;
      }
      ...
      console.error(`Erro processando action ${action.type}:`, error);
  ...
  loadIngredients: async () => {
    ...
    try {
      ...
      set(state => ({ ingredients: { ...state.ingredients, all: allIngredients } }));
    } catch (error) {
      // handle error silently
    } finally {
