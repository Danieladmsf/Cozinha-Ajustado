...
export function calculateTotalWeight(items) {
  if (!items || !Array.isArray(items)) {
    console.log("[WeightCalculator] No items provided, returning 0.");
    return 0;
  }

  let totalWeight = 0;
  let totalCarnesWeight = 0;

  console.log("--- Calculating Total Weight ---");
  items.forEach(item => {
    const itemWeight = calculateItemWeight(item);
    const unitType = (item.unit_type || '').toLowerCase();
    const isCarne = item.category && item.category.toLowerCase().includes('carnes');

    // Log if item has 0 weight after calculation
    if (itemWeight === 0) {
      console.log(`[WeightCalculator] Item Skipped (Zero Weight): ${item.recipe_name} - ${item.quantity} ${item.unit_type}`);
      return; // Skip items that result in 0 weight
    }

    totalWeight += itemWeight;
    console.log(`[WeightCalculator] Item Included: ${item.recipe_name} - ${item.quantity} ${item.unit_type} -> ${itemWeight.toFixed(3)} kg`);

    if (isCarne) {
      totalCarnesWeight += itemWeight;
      console.log(`[WeightCalculator]   (Carne) ${item.recipe_name} added to carnes total.`);
    }
  });

  console.log(`--- Total Carnes Weight: ${totalCarnesWeight.toFixed(3)} kg ---`);
  console.log(`--- Final Total Weight: ${totalWeight.toFixed(3)} kg ---`);
